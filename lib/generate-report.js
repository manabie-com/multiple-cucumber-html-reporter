			...
			if (step.embeddings !== undefined) {
				...
				step.embeddings.forEach((embedding, embeddingIndex) => {
					...
					if (embedding.mime_type === 'application/json' || embedding.media && embedding.media.type === 'application/json') {
						data = typeof embedding.data === 'string' ? JSON.parse(embedding.data) : embedding.data;
						step.json = (step.json ? step.json : []).concat([data]);
						data = '<pre class=info>' + data + '</pre>';
					} else if (embedding.mime_type === 'text/html' || (embedding.media && embedding.media.type === 'text/html')) {
						data = _isBase64(embedding.data) ? Base64.decode(embedding.data) :
							embedding.data;
						step.html = (step.html ? step.html : []).concat([data]);
						data = '<div class=info>' + data + '</div>';
					} else if (embedding.mime_type === 'text/plain' || (embedding.media && embedding.media.type === 'text/plain')) {
						data = _isBase64(embedding.data) ? _escapeHtml(Base64.decode(embedding.data)) : _escapeHtml(embedding.data);
						step.text = (step.text ? step.text : []).concat([data]);
						data = '<pre class=info>' + data + '</pre>';
					} else if (embedding.mime_type === 'image/png' || (embedding.media && embedding.media.type === 'image/png')) {
						data = 'data:image/png;base64,' + embedding.data;
						step.image = (step.image ? step.image : []).concat(['data:image/png;base64,' + embedding.data]);
						step.embeddings[embeddingIndex] = 'data:image/png;base64,' + embedding.data;
						data = '<img class=screenshot style="margin-bottom:16px;" src="' + data + '"/>';
					} else {
					...
					step.attachmentsHtml.push(data.replaceAll('\\"', '"'));
				});
			}
